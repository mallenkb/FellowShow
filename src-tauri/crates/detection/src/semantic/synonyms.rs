...
impl SynonymExpander {
    ...
    #[expect(
        ...
        reason = "synonym table is a large data definition"
    )]
    pub fn new() -> Self {
...
mod tests {
    ...
    fn test_synonym_expansion() {
        ...
        assert!(
            variants.len() > 1,
            "Expected more than 1 variant, got {}",
            variants.len()
        );
    ...
    fn test_worship_synonyms() {
        ...
        assert!(variants.len() > 1);
        assert!(
            variants.iter().any(|v| v.contains("praise")
                || v.contains("glorify")
                || v.contains("adore")
                || v.contains("exalt")),
    ...
    fn test_death_synonyms() {
        ...
        assert!(variants.len() > 1);
        assert!(
            variants.iter().any(|v| v.contains("die")
                || v.contains("perish")
