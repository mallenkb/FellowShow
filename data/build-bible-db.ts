...
function main() {
    ...
    requestedTranslations.size > 0
      ? [
          ...requestedTranslationOrder
            .map((abbreviation) =>
              TRANSLATIONS_META.find(
                (meta) => meta.abbreviation === abbreviation
              )
            )
            .filter((meta): meta is (typeof TRANSLATIONS_META)[number] =>
              Boolean(meta)
            ),
          ...(includeUndownloadedTranslations
            ? TRANSLATIONS_META.filter(
                (meta) => !requestedTranslations.has(meta.abbreviation)
              )
            : []),
        ]
      : TRANSLATIONS_META
  ...
  if (requestedTranslations.size > 0) {
    const knownTranslations = new Set(
      TRANSLATIONS_META.map((meta) => meta.abbreviation)
    )
    const unknownTranslations = [...requestedTranslations].filter(
      (translation) => !knownTranslations.has(translation)
    )
    if (unknownTranslations.length > 0) {
      throw new Error(
        `Unknown translation(s): ${unknownTranslations.join(", ")}`
      )
    }
    console.log(
      `  Building selected translations: ${[...requestedTranslations].join(", ")}`
    )
  }
